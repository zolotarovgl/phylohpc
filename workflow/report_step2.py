...
.col-tri{fill:#ffffff;stroke:#222;stroke-width:1px;cursor:pointer}
...
<div id="body-wrap">
  ...
  <div class="tab-pane active" id="pane-heatmap">
    <div id="hm-custom-bar" style="display:none;flex-shrink:0;border-bottom:1px solid #ddd;background:#fffbf0;padding:4px 10px;gap:6px;flex-wrap:wrap;align-items:center;position:relative">
      <span style="font-size:11px;color:#888;font-weight:600">Custom OG heatmap:</span>
      <div style="position:relative;display:inline-block">
        <input id="hm-og-search" type="text" placeholder="&#128269; Search OG or gene name&#8230;" autocomplete="off" style="font-size:11px;padding:2px 8px;border:1px solid #ccc;border-radius:3px;width:240px" oninput="hmOGSearchInput(this.value)" onkeydown="hmOGSearchKey(event)">
        <ul id="hm-og-dropdown" style="display:none;position:absolute;top:100%;left:0;z-index:200;background:#fff;border:1px solid #ccc;border-radius:0 0 4px 4px;list-style:none;margin:0;padding:0;max-height:220px;overflow-y:auto;min-width:300px;box-shadow:0 3px 8px rgba(0,0,0,.12);font-size:11px"></ul>
      </div>
    ...
    <div id="hm-layout">
      ...
      <div id="hm-right" style="display:flex;flex-direction:column;flex:1;overflow:hidden;min-width:0">
        ...
        <div id="hm-col-strip" style="display:flex;align-items:center;gap:10px;padding:3px 10px;background:#fafafa;border-bottom:1px solid #eee;flex-shrink:0;flex-wrap:wrap">
          ...
          <input type="text" id="hm-text-search" placeholder="&#128269; Search…" style="font-size:11px;padding:2px 7px;border:1px solid #ccc;border-radius:3px;width:140px" oninput="hmTextFilter=this.value.toLowerCase().trim();drawHeatmap()">
          <button id="btn-hm-custom" onclick="hmEnterCustom()" style="padding:2px 8px;font-size:11px;border:1px solid #4a90d9;color:#4a90d9;border-radius:3px;background:#fff;cursor:pointer;font-weight:600">&#9998; Custom</button>
          <details style="margin-left:auto;font-size:10px">
            ...
            <div style="position:absolute;z-index:50;background:#fff;border:1px solid #ddd;border-radius:4px;padding:6px 10px;box-shadow:0 2px 8px rgba(0,0,0,.12);line-height:1.7;color:#999;min-width:300px;right:10px">
              ...
              <div><b style="color:#777">Custom:</b> build a heatmap from any combination of orthogroups; search by OG name or gene ID</div>
            </div>
          </details>
        </div>
...
let hmCustomOGs    = [];     // user-selected OG names for custom view
  ...
  function drawCollapsedTriangles(n){
    ...
    if(cladoCollapsed.has(key)){
      ...
      svg.append("polygon")
        ...
        .on("click",()=>{ cladoCollapsed.delete(key); hideTip(); drawCladogram(); if(document.getElementById("pane-heatmap").classList.contains("active")) drawHeatmap(); });
...
function drawHeatmap() {
  ...
  if (hmViewMode==="class") {
    ...
    clickHandler=(_ev,_d,sp)=>{
      ...
      // always highlight the OG and activate focus/hide mode
      ogHlQueries=[_d.id]; rebuildOgHlSet();
      if(sp && _d.species_counts[sp]){ hlQueries=[sp]; rebuildHlSet(); }
      hideNonHl=true;
      const chkHide=document.getElementById("chk-hide-nonhl");
    ...
    if(!hmCustomOGs.length){
      document.getElementById("heatmap-panel").innerHTML=
        ...
        'You can search by OG name or paste a gene name to find its OG.</p>';
      return;
    }
    data=hmCustomOGs.map(og=>{
      const r=hmOGIndex[og]||{};
      return {id:og,species_counts:r.species_counts||{},total:r.total||0,family:r.family||"",cls:r.cls||""};
    });
    back.style.display="none";
    crumb.textContent="Custom selection ("+hmCustomOGs.length+" OG"+(hmCustomOGs.length!==1?"s":"")+")";
    colLabel=d=>d.id;
    clickHandler=(_ev,d,sp)=>{
      const tRec=hmOGIndex[d.id]?TREE_INDEX.find(t=>t.id===hmOGIndex[d.id].hgId):null;
      if(!tRec) return;
      switchTab("trees"); selectTree(tRec); renderSidebar("");
      ogHlQueries=[d.id]; rebuildOgHlSet();
      if(sp && d.species_counts[sp]){ hlQueries=[sp]; rebuildHlSet(); }
      hideNonHl=true;
      const chkHide=document.getElementById("chk-hide-nonhl");
      if(chkHide) chkHide.checked=true;
      setTimeout(focusHighlighted,80);
...
function hmEnterCustom(){
  buildOGIndex();
  hmViewMode="custom";
  document.getElementById("hm-custom-bar").style.display="flex";
  document.getElementById("hm-og-search").focus();
  drawHeatmap();
}
function hmExitCustom(){
  ...
  document.getElementById("hm-og-dropdown").style.display="none";
  drawHeatmap();
}
function hmCustomClear(){
  hmCustomOGs=[]; renderCustomChips(); drawHeatmap();
}

let _hmOGSearchHits=[], _hmOGSearchSel=-1;

function hmOGSearchInput(val){
  val=(val||"").trim();
  const dd=document.getElementById("hm-og-dropdown");
  if(!val){ dd.style.display="none"; return; }
  buildOGIndex();
  const q=val.toLowerCase();
  // OG name matches
  const ogHits=Object.keys(hmOGIndex)
    .filter(k=>k.toLowerCase().includes(q))
    .sort((a,b)=>{
      const as=a.toLowerCase().startsWith(q),bs=b.toLowerCase().startsWith(q);
      return (as===bs)?a.localeCompare(b):(as?-1:1);
    }).slice(0,40).map(og=>({og,matchedGene:null}));
  // gene name matches → resolved to OG
  const seenOGs=new Set(ogHits.map(h=>h.og));
  const geneHits=Object.entries(hmGeneIndex)
    .filter(([g])=>g.toLowerCase().includes(q))
    .filter(([,og])=>!seenOGs.has(og))
    .sort((a,b)=>a[0].localeCompare(b[0])).slice(0,20)
    .map(([g,og])=>{ seenOGs.add(og); return {og,matchedGene:g}; });
  _hmOGSearchHits=[...ogHits,...geneHits];
  _hmOGSearchSel=-1;
  dd.innerHTML=_hmOGSearchHits.map((hit,i)=>{
    const {og,matchedGene}=hit;
    const r=hmOGIndex[og]||{family:"",total:0};
    const already=hmCustomOGs.includes(og);
    const escapedOg=og.replace(/\\\\/g,"\\\\\\\\").replace(/'/g,"\\\\'");
    const label=matchedGene
      ? `<span style="color:#888;font-size:10px">gene</span> <b>${matchedGene}</b><span style="color:#aaa;font-size:10px"> \u2192 ${og} \u00b7 ${r.family}</span>`
      : `<b>${og}</b><span style="color:#aaa;font-size:10px"> \u00b7 ${r.family} \u00b7 ${r.total} genes</span>`;
    return `<li data-i="${i}" style="padding:3px 10px;cursor:pointer;background:${already?"#e8f5e9":"#fff"};color:${already?"#2e7d32":"#222"}" onmousedown="hmOGSearchAdd('${escapedOg}')">${label}${already?`<span style="color:#27ae60;float:right">&#10003;</span>`:""}</li>`;
  }).join("")||`<li style="padding:4px 10px;color:#aaa">No matches</li>`;
  dd.style.display="block";
}

function hmOGSearchKey(ev){
  const dd=document.getElementById("hm-og-dropdown");
  const items=dd.querySelectorAll("li[data-i]");
  if(ev.key==="ArrowDown"){
    ev.preventDefault();
    _hmOGSearchSel=Math.min(_hmOGSearchSel+1,items.length-1);
    items.forEach((li,i)=>li.style.background=i===_hmOGSearchSel?"#e3f0ff":(hmCustomOGs.includes((_hmOGSearchHits[i]||{}).og)?"#e8f5e9":"#fff"));
  } else if(ev.key==="ArrowUp"){
    ev.preventDefault();
    _hmOGSearchSel=Math.max(_hmOGSearchSel-1,0);
    items.forEach((li,i)=>li.style.background=i===_hmOGSearchSel?"#e3f0ff":(hmCustomOGs.includes((_hmOGSearchHits[i]||{}).og)?"#e8f5e9":"#fff"));
  } else if(ev.key==="Enter"){
    ev.preventDefault();
    const idx=_hmOGSearchSel>=0?_hmOGSearchSel:(_hmOGSearchHits.length===1?0:-1);
    if(idx>=0) hmOGSearchAdd(_hmOGSearchHits[idx].og);
  } else if(ev.key==="Escape"){
    dd.style.display="none";
  }
}

function hmOGSearchAdd(og){
  if(!hmCustomOGs.includes(og)) hmCustomOGs.push(og);
  const inp=document.getElementById("hm-og-search");
  inp.value=""; inp.focus();
  document.getElementById("hm-og-dropdown").style.display="none";
...
function renderCustomChips(){
  document.getElementById("hm-custom-chips").innerHTML=hmCustomOGs.map((og,i)=>`<span style="display:inline-flex;align-items:center;gap:3px;padding:1px 7px;border-radius:10px;background:#4a90d9;color:#fff;font-size:10px">${og}<span style="cursor:pointer;margin-left:2px;opacity:.8" onclick="hmCustomOGs.splice(${i},1);renderCustomChips();drawHeatmap()">&#10005;</span></span>`).join("");
}
...
document.addEventListener("click",ev=>{
  ...
  // close OG search dropdown when clicking outside
  if(!ev.target.closest("#hm-og-search")&&!ev.target.closest("#hm-og-dropdown")){
    const dd=document.getElementById("hm-og-dropdown");
...
function focusHighlighted(){
  if((!hlSet&&!ogHlSet)||!rootNode)return;
  // expand all
  ...
  // a leaf passes if it satisfies BOTH active filters (species AND/OR OG)
  const hasHl=new Map();
  rootNode.eachAfter(d=>{
    if(d.data.leaf){
      const sp=d.data.species||"";
      const gid=d.data.gene_id||d.data.name;
      const og=d.data.og||ogGene2Name[gid]||"";
      const spOk=!hlSet||hlSet.has(sp);
      const ogOk=!ogHlSet||ogHlSet.has(og);
      hasHl.set(d,spOk&&ogOk);
    } else {
