...
function selectTree(rec){
  ...
  cladeSp2Color={}; cladeSp2Group={}; cladeGrpColor={}; ogLeaf2Color={}; ogName2Color={};
  colorMode="og";
  ...
  Object.keys(_ogs).sort().forEach((og,i)=>{
    ...
    for(const gid of _ogs[og]) ogLeaf2Color[gid]=col;
  });
...
function rebuildOgColors(){
  ...
  ogLeaf2Color={}; ogName2Color={};
  const ogs=activeOgs();
  Object.keys(ogs).sort().forEach((og,i)=>{
    const col=palette[i%palette.length]; ogName2Color[og]=col;
    for(const gid of ogs[og]) ogLeaf2Color[gid]=col;
  });
}
...
function renderTree(animate){
  ...
  nodeSel.select("circle")
    ...
    .attr("fill",d=>{
      if(d.data.leaf){
        if(colorMode==="og") return ogLeafColor(d.data.gene_id||d.data.name);
  ...
  // leaf labels (gene_id only for pipe-format names)
  nodeSel.select(".leaf-label")
    .attr("x",7).attr("dy","0.32em").attr("text-anchor","start")
    .attr("font-size",d=>d.data.leaf?Math.min(11,rowH-2):0)
    .attr("fill",d=>colorMode==="og"?ogLeafColor(d.data.gene_id||d.data.name):leafColor(d.data.species||""))
    .attr("display",d=>d.data.leaf?null:"none")
    .text(d=>d.data.leaf?(d.data.gene_id||d.data.name):"");
