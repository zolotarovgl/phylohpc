...
<div id="body-wrap">
  ...
  <div class="tab-pane active" id="pane-heatmap">
    <div id="hm-custom-bar" style="display:none;position:fixed;z-index:500;border:1px solid #c8b96e;border-top:none;background:#fffbf0;padding:6px 12px;gap:6px;flex-wrap:wrap;align-items:center;box-shadow:0 4px 12px rgba(0,0,0,.15);border-radius:0 0 6px 6px">
      <span style="font-size:11px;color:#888;font-weight:600">Search / select OGs:</span>
    ...
    <div id="hm-layout">
      ...
      <div id="hm-right" style="display:flex;flex-direction:column;flex:1;overflow:hidden;min-width:0">
        ...
        <div id="hm-col-strip" style="display:flex;align-items:center;gap:10px;padding:3px 10px;background:#fafafa;border-bottom:1px solid #eee;flex-shrink:0;flex-wrap:wrap">
          <div style="position:relative;display:inline-block">
            <input id="hm-text-search" type="text" placeholder="&#128269; Search classes, families, HGs&hellip;" autocomplete="off"
          ...
          <button id="btn-hm-custom" onclick="hmEnterCustom()" style="padding:2px 8px;font-size:11px;border:1px solid #4a90d9;color:#4a90d9;border-radius:3px;background:#fff;cursor:pointer;font-weight:600">&#128269; Search OGs</button>
...
function hmTextSearchInput(val){
  ...
  _hmSearchHits=hmSearchIndex.filter(r=>r.label.toLowerCase().includes(q)).slice(0,30);
  _hmSearchSel=-1;
  dd.innerHTML=_hmSearchHits.map((h,i)=>`<div data-i="${i}"
    style="padding:4px 10px;cursor:pointer;border-bottom:1px solid #f0f0f0;
    color:${h.type==="hg"?"#333":h.type==="family"?"#555":"#888"}"
    onmousedown="hmTextSearchGo(${i})">${h.label}</div>`).join("")
    ||`<div style="padding:6px 10px;color:#aaa">No matches</div>`;
  dd.style.display="block";
}
function hmTextSearchKey(ev){
  const dd=document.getElementById("hm-search-dd");
  const items=dd.querySelectorAll("[data-i]");
  if(ev.key==="ArrowDown"){ ev.preventDefault(); _hmSearchSel=Math.min(_hmSearchSel+1,items.length-1); items.forEach((el,i)=>el.style.background=i===_hmSearchSel?"#e3f0ff":""); }
  else if(ev.key==="ArrowUp"){ ev.preventDefault(); _hmSearchSel=Math.max(_hmSearchSel-1,0); items.forEach((el,i)=>el.style.background=i===_hmSearchSel?"#e3f0ff":""); }
  else if(ev.key==="Enter"){ const idx=_hmSearchSel>=0?_hmSearchSel:(_hmSearchHits.length===1?0:-1); if(idx>=0){hmTextSearchGo(idx);ev.preventDefault();} }
  else if(ev.key==="Escape"){ dd.style.display="none"; }
}
function hmTextSearchGo(i){
  const h=_hmSearchHits[i]; if(!h) return;
...
function hmOGSearchInput(val){
  ...
  // ── group hits: class and family that have OGs matching the query ──────
  ...
  const clsHits=[...clsMap.entries()]
    .filter(([k])=>k.toLowerCase().includes(q))
    .sort((a,b)=>b[1].length-a[1].length).slice(0,8)
    .map(([k,ogs])=>({type:"group",groupType:"class",key:k,ogs,count:ogs.length}));
  const famHits=[...famMap.entries()]
    .filter(([k])=>k.toLowerCase().includes(q))
    .sort((a,b)=>b[1].length-a[1].length).slice(0,8)
    .map(([k,ogs])=>({type:"group",groupType:"family",key:k,ogs,count:ogs.length}));

  // ── OG name matches — exact-start first, then by descending gene count ─
  const ogHits=Object.keys(hmOGIndex)
    .filter(k=>k.toLowerCase().includes(q))
    .sort((a,b)=>{
      const as=a.toLowerCase().startsWith(q),bs=b.toLowerCase().startsWith(q);
      if(as!==bs) return as?-1:1;
      return (hmOGIndex[b].total||0)-(hmOGIndex[a].total||0);
    }).slice(0,40).map(og=>({type:"og",og,matchedGene:null}));
  ...
  const geneHits=Object.entries(hmGeneIndex)
    ...
    .sort((a,b)=>(hmOGIndex[b[1]]?.total||0)-(hmOGIndex[a[1]]?.total||0)).slice(0,20)
    .map(([g,og])=>{ seenOGs.add(og); return {type:"og",og,matchedGene:g}; });

  _hmOGSearchHits=[...clsHits,...famHits,...ogHits,...geneHits];
  _hmOGSearchSel=-1;
  dd.innerHTML=_hmOGSearchHits.map((hit,i)=>{
    if(hit.type==="group"){
      const alreadyAll=hit.ogs.every(og=>effOGs.has(og));
      const existsGroup=hmCustomGroups.some(g=>g.groupType===hit.groupType&&g.key===hit.key);
      const done=alreadyAll||existsGroup;
      const icon=hit.groupType==="class"?"\uD83D\uDCC2":"\uD83E\uDDF9";
      const label=`${icon} <b>${hit.key}</b><span style="color:#aaa;font-size:10px"> \u00b7 ${hit.groupType} \u00b7 ${hit.count} OGs</span>`;
      return `<li data-i="${i}" style="padding:3px 10px;cursor:pointer;background:${done?"#e8f5e9":"#f0f6ff"};color:${done?"#2e7d32":"#1a4a7a"}" onmousedown="hmOGSearchSelect(${i})">${label}${done?`<span style="color:#27ae60;float:right">&#10003;</span>`:""}</li>`;
    }
    const {og,matchedGene}=hit;
    const r=hmOGIndex[og]||{family:"",total:0};
    const already=effOGs.has(og);
    ...
    return `<li data-i="${i}" style="padding:3px 10px;cursor:pointer;background:${already?"#e8f5e9":"#fff"};color:${already?"#2e7d32":"#222"}" onmousedown="hmOGSearchSelect(${i})">${label}${already?`<span style="color:#27ae60;float:right">&#10003;</span>`:""}</li>`;
...
function hmOGSearchKey(ev){
  ...
  if(ev.key==="ArrowDown"){
    ...
    items.forEach((li,i)=>li.style.background=i===_hmOGSearchSel?"#e3f0ff":li.dataset.orig||"#fff");
    ...
    if(idx>=0) hmOGSearchSelect(idx);
  } else if(ev.key==="Escape"){
    dd.style.display="none";
  }
}

function hmOGSearchSelect(i){
  const hit=_hmOGSearchHits[i];
  if(!hit) return;
  if(hit.type==="group"){
    if(!hmCustomGroups.some(g=>g.groupType===hit.groupType&&g.key===hit.key))
      hmCustomGroups.push({groupType:hit.groupType,key:hit.key,label:hit.key,ogs:hit.ogs});
  } else {
    if(!hmCustomOGs.includes(hit.og)) hmCustomOGs.push(hit.og);
  }
...
// keep backward compat (called by old onmousedown attributes still in the page after first load)
