...
function drawGeneTree(treeData){
  ...
  // reset reroot state
  _rerootMode=false; _isRerooted=false; _origTreeDictForReroot=null;
...
function renderTree(animate){
  ...
  nodeSel.select("circle")
    ...
    .on("click",(event,d)=>{
      event.stopPropagation();
      if(_rerootMode){ hideTip(); rerootAtNode(d); return; }
...
// ── reroot ──────────────────────────────────────────────────────────────────
let _rerootMode=false;
let _origTreeDictForReroot=null;   // saved before the first reroot
let _isRerooted=false;
function toggleRerootMode(){
  _rerootMode=!_rerootMode;
  document.getElementById("btn-reroot").classList.toggle("active-btn",_rerootMode);
  document.getElementById("tree-wrap").style.cursor=_rerootMode?"crosshair":"";
}
function resetRoot(){
  if(!_origTreeDictForReroot)return;
  _isRerooted=false;
  document.getElementById("btn-reset-root").style.display="none";
  if(_rerootMode){ _rerootMode=false; document.getElementById("btn-reroot").classList.remove("active-btn"); document.getElementById("tree-wrap").style.cursor=""; }
  drawGeneTree(_origTreeDictForReroot);
  _origTreeDictForReroot=null;
}
// Convert a d3-hierarchy node (and its current children/collapsed children) back to a plain dict
function _h2d(d){
...
// Build the "above" part of the tree when rerooting at `node`:
// returns a plain dict representing node.parent with `node` removed,
// and the ancestor chain reversed so grandparents become children.
function _rerootAbove(node){
  const p=node.parent;
  if(!p) return null;
  const siblings=(p.children||[]).filter(c=>c!==node);
  const children=siblings.map(_h2d);
  if(p.parent) children.push(_rerootAbove(p));
  const obj=Object.assign({},p.data);
  obj.children=children;
  return obj;
}
function rerootAtNode(d){
  if(!rootNode||!d.parent) return;   // can't reroot at existing root
  // save original only on the first reroot
  if(!_isRerooted) _origTreeDictForReroot=_h2d(rootNode);
  // expand all collapse state so we work with the full topology
  rootNode.each(n=>{if(n._children){n.children=n._children;n._children=null;}});
  const above=_rerootAbove(d);
  const newRoot={name:"",leaf:false,children:[_h2d(d)]};
