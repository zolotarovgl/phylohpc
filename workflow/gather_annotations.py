...
def parse_args():
    ...
    parser.add_argument("--tree-dir", help="Directory with POSSVM outputs - e.g. tfs.Forkhead.HG1.tree.ortholog_groups.csv")
...
def collect_tmp_anno(tree_dir, prefix, species_id):
    result = {}
    tree_dir = Path(tree_dir)

    # If prefix is None or empty → use all matching files
    if not prefix:
        pattern = "*groups.csv"
    else:
...
def collect_pep2hg(cluster_dir, species_id):
    ...
    cluster_dir : str or Path
        Directory containing *_cluster.tsv files
    species_id : str
        Prefix to filter genes (e.g. species prefix)

    Returns
    -------
    dict
        {gene_id: cluster_label}
        where cluster_label = PREF.clusterID
    """
...
def main():
    ...
    if not args.search_dir or not args.tree_dir:
            sys.exit("Provide either --config OR both --search-dir and --tree-dir")
    else:
    ...
    with tempfile.TemporaryDirectory(prefix="gather_anno_"):
        ...
        tmp_anno = collect_tmp_anno(tree_dir, prefix, species_id)
